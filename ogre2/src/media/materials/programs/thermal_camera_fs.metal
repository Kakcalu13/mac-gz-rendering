    ...
    constant Params &p                        [[buffer(PARAMETER_SLOT)]])
{
    ...
    float d = -viewSpacePos.z;
    d = (d - p.near) / (p.far - p.near);
    ...
    float heat = colorTexture.sample(colorSampler, input.uv0).x;
    if (heat > 0.0f)
    {
        // heat is normalized so convert back to kelvin
        temp = heat * 655.35f;
        heatRange = p.heatSourceTempRange;
    }

    // simulate temp variation as a function of depth
    float delta = (1.0f - d) * heatRange;
    temp = temp - heatRange / 2.0f + delta;
    // note: clamp result intentionally unused to match original GLSL behavior
    clamp(temp, p.min, p.max);

    // apply resolution factor and normalize
    temp /= p.resolution;
    temp /= 65535.0f;
